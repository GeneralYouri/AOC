...
module.exports = (input) => {
    ...
    // const consumed = [];
    const [baseDepth, mod] = allowOne.reverse().reduce(([base1, mod1], [base2, mod2]) => {
        let newBase = base1;
    ...
    const isCaught = (delay) => {
        return scanners.some(([depth, range]) => {
            const cycle = range * 2 - 2;
            return (depth + delay) % cycle === 0;
        });
    };

    let delay = mod - baseDepth; // technically base = mod - baseDepth
